...
public class MatrixLogicMiniMaxAB {
	...
	int ABsearch(Board board){
			...
			for (int a: Actions(board)){
				int b=MinValue(Result(board,a,1),Integer.MIN_VALUE,Integer.MAX_VALUE);
				if(b==v) {
					choice=a;
					}
			}
	...
	int MaxValue(Board board,int alpha,int beta){
		...
		for (int a:Actions(board)){
			v=Max(v,MinValue((Result(board,a,1)),alpha,beta));
			if(v>=beta){return v;}
			alpha=Max(alpha,v);
	...
	int MinValue(Board board,int alpha,int beta){
		...
		for (int a:Actions(board)){
			v=Min(v,MaxValue((Result(board,a,-1)),alpha,beta));
			if(v<=alpha){return v;}
			beta = Min(beta,v);
			if(counter>cutoff){break;}
		}
